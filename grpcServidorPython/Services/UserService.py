...
# O gRPC automaticamente espera que o método tenha dois parâmetros: request e context, mesmo que não use o context, ele precisa estar ali.
    def AddUser(self, request, context):
        ...
        print('Usuário adicionado:', request.idade)
        return pb2.UserResponse(**{'id': id, 'name' : request.name, 'email': request.email, 'idade': request.idade, 'msg': 'add ok'})
    ...
    def GetUser(self, request, context):
        ...
        if user:
            return pb2.UserResponse(**{'id': user['id'], 'name' : user['name'], 'email': user['email'], 'idade': user['idade'], 'msg': 'get ok'})
        else:
            return pb2.UserResponse(**{'id': 0, 'name' : '', 'email': '', 'idade': '', 'msg': 'get erro'})
    ...
    def UpdateUser(self, request, context):
        ...
        if user:
            ...
            return pb2.UserResponse(**{'id': user['id'], 'name' : user['name'], 'email': user['email'], 'idade': user['idade'], 'msg': 'update ok'})
        else:
            return pb2.UserResponse(**{'id': 0, 'name' : '', 'email': '', 'idade': '', 'msg': 'update erro'})
    ...
    def DeleteUser(self, request, context):
        ...
        if user:
            ...
            return pb2.UserResponse(**{'id': user['id'], 'name' : user['name'], 'email': user['email'], 'idade': user['idade'], 'msg': 'del ok'})
        else:
            return pb2.UserResponse(**{'id': 0, 'name' : '', 'email': '', 'idade': '', 'msg': 'del erro'})
